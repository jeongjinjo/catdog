...
public class TodoController {
    ...
    @Operation(summary = "반려동물의 할일 등록"
            , description = "할일관리를 만들어주기 위한 INSERT 기능")
    @PostMapping()
    ...
    @Operation(summary = "반려동물의 할일 조회"
            , description = "반려동물의 고유번호로 할일 조회. SELECT 기능")
    @GetMapping()
    public ResponseEntity<Map<Integer, List<Object>>> todolist(@RequestParam String member_id, Integer petNum, Date date){
        Map<Integer, List<Object>> todolist = todoService.getdatePetTodoList(member_id, petNum, date);
    ...
    @Operation(summary = "반려동물의 할일 수정"
            , description = "반려동물 고유번호를 이용해 오늘 날짜일 경우 수정가능. UPDATE 기능")
    @PutMapping()
